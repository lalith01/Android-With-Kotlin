...
class checkbox : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val xml = cbxmlfragment()
        val kotlin = cbkotlinfragment()
        val play = cbplayfragment()

        setCurrentFragment(xml)
        cb_bottom_nav.setOnNavigationItemSelectedListener {
            when(it.itemId)
            {
                R.id.bottom_nav_xml ->
                {
    ...
    private fun setCurrentFragment(fragment: Fragment) = supportFragmentManager.beginTransaction().apply {
        replace(R.id.cb_fragment_layout,fragment)
        commit()
    }
}