...
class custom_toast : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        actionBar!!.title = "Custom Toast"
        getSupportActionBar()?.setTitle(Html.fromHtml("<font color=\"#14171A\">" + actionBar!!.title + "</font>"));
        actionBar.setDisplayHomeAsUpEnabled(true)

        val xml = customtoastxmlfragment()
        val kotlin = customtoastkotlinfragment()
        val play = customtoastplayfragment()
        setCurrentFragment(xml)

        custom_toast_bottom_nav.setOnNavigationItemSelectedListener {
            when(it.itemId)
            {
                R.id.bottom_nav_xml ->
                {
    ...
    private fun setCurrentFragment(fragment: Fragment) = supportFragmentManager.beginTransaction().apply {
        replace(R.id.custom_toast_fragment_layout,fragment)
        commit()
    }
}