...
class imageView : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val xml = imgxmlfragment()
        val kotlin = imgkotlinfragment()
        val play = imgplayfragment()

        setCurrentFragment(xml)
        img_bottom_nav.setOnNavigationItemSelectedListener {
            when(it.itemId)
            {
                R.id.bottom_nav_xml ->
                {
    ...
    private fun setCurrentFragment(fragment: Fragment) = supportFragmentManager.beginTransaction().apply {
        replace(R.id.img_fragment_layout,fragment)
        commit()
    }
}